...
def convert_data(trainingdata_dir,
  ...
  for filename in sorted(tf.gfile.ListDirectory(trainingdata_dir)):
    if not filename.endswith(".csv"):
      ...
      continue
    file_handles.append(csv.DictReader(open(os.path.join(trainingdata_dir, filename))))
  ...
  import tqdm
  for c in tqdm.tqdm(reading_order):
    line = file_handles[c].next()
    ink = None
    while ink is None:
